...
- (UIImage*)imageForQRString:(NSString*)qrString size:(CGSize)size {
    ...
    if (qrFilter) {
        ...
        // Send the image back
        return [self createNonInterpolatedUIImageFromCIImage:qrFilter.outputImage withScale:2*[[UIScreen mainScreen] scale]];
    } else {
        return nil;
    }
}
...
- (void) viewDidLoad {
    ...
    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:@"Listo" style:UIBarButtonItemStyleDone target:self action:@selector(doneButtonPressed:)];
